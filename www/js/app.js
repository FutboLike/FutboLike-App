...
nativePlayer()?.addListener("playerError", event => {
  lastPlayerError = `ExoPlayer: ${event?.message || "error desconocido"}`;
...
async function attachExoPlayerSource(url) {
  lastPlayerError = "";
  try {
    ...
    showError("ExoPlayer no pudo iniciar la transmisión.");
  }
}
